...
<div style="min-height: 300px;max-width: 1000px;margin: auto;">
	...
	<!--profile data-->
<?php if(is_object($data['profile_data'])): ?>
	...
	<?php if(is_array($orders)):?>
			...
			<table class="table">
				<thead>
					<tr><th>Order no</th><th>Order date</th><th>Total</th><th>Delivery Address</th><th>City/State</th><th>Mobile Phone</th><th>...</th></tr>
				</thead>
				<tbody onclick="show_details(event)">
					<?php foreach($orders as $order):?>

						<tr style="position: relative;"><td><?=$order->id?></td><td><?=date("jS M Y H:i a",strtotime($order->date))?></td><td>$<?=$order->total?></td><td><?=$order->delivery_address?></td><td><?=$order->state?></td><td><?=$order->mobile_phone?></td>
							<td>
